...
use zr_nbt::Nbt;
use zr_network::{handler::{error::*, ClientNetworkHandler}, packet::PacketData};
...
pub struct ClientProtocolHandler {
    client_handler: ClientNetworkHandler,
    state: State,
    user: User,
}

impl ClientProtocolHandler {
    pub fn new(client_handler: ClientNetworkHandler) -> Self {
        Self { client_handler, state: State::Handshake, user: User::new() }
    }

    async fn handshake(&mut self) -> Result<()> {
        let handshake = self.client_handler.read::<handshake::Handshake>().await?;
    ...
    async fn status(&mut self) -> Result<()> {
        loop {
            match self.client_handler.read_packet().await {
                Ok(packet) => {
                    match packet.id() {
                        status::client::Ping::ID => {
                            let ping = packet.data::<status::client::Ping>().map_err(|error| NetworkError::PacketError(error))?;
                            self.client_handler.write::<status::server::Pong>(status::server::Pong { payload: ping.payload }).await?;
                        },
                        status::client::StatusRequest::ID => {
                            ...
                            self.client_handler.write::<status::server::StatusResponse>(status::server::StatusResponse { json_response: JSON.to_string() }).await?;
                        }
                ...
                Err(err) => if let NetworkError::ConnectionClose = err {
                    return Ok(()); // better way to exit without error ? maybe 
                } else {
                    return Err(err);
                }
            }
    ...
    async fn config(&mut self) -> Result<()> {
        let packet = self.client_handler.read_packet().await?;
        self.client_handler.write(config::server::RegistryData { registry_codec: Nbt::new(self.get_registry_data_codec()) }).await?;
        match packet.id() {
    ...
    async fn set_compression(&mut self) -> Result<()> {
        if self.client_handler.get_options().is_compression_set() {
            ...
            self.client_handler.write(login::server::SetCompression { threshold }).await?;
    ...
    async fn login_success(&mut self) -> Result<()> {
        self.client_handler.write(login::server::LoginSuccess { uuid: self.user.uuid.unwrap(), username: self.user.username.clone().unwrap(), properties: Vec::new() }).await?;
        Ok(())
    }
