...
pub trait Binary {
    ...
    fn from_binary(bin: Vec<u8>) -> Result<Self> where Self: Sized;
}
...
impl Binary for i64 {
    ...
    fn from_binary(bin: Vec<u8>) -> Result<Self> where Self: Sized {
        if bin.len() < 8 {
            ...
            Ok(Self::from_be_bytes([bin[0], bin[1], bin[2], bin[3], bin[4], bin[5], bin[6], bin[7]]))
        }
    }
}
...
impl Binary for i128 {
    ...
    fn from_binary(bin: Vec<u8>) -> Result<Self> where Self: Sized {
        if bin.len() < 16 {
            ...
            Ok(Self::from_be_bytes([bin[0], bin[1], bin[2], bin[3], bin[4], bin[5], bin[6], bin[7], bin[8], bin[9], bin[10], bin[11], bin[12], bin[13], bin[14], bin[15]]))
        }
    }
}
...
impl Binary for String {
    ...
    fn from_binary(bin: Vec<u8>) -> Result<Self> where Self: Sized
    {
        ...
        let utf8 =
            String::from_utf8_lossy(&bin[cursor..(cursor + len.0 as usize)]).to_string();
        Ok(utf8)
    }
}
